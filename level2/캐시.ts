...
class Cache {
  ...
  private _store: { [x: string]: number }
  ...
  dbGet( cityName: string ) {
    if ( cityName in this._store ) {
      this._store[cityName] = this.getTime()
      return 1
    }

    this._store[cityName] = this.getTime()

    if ( this.storeSize > this._size ) {
      let storeEntries = Object.entries( this._store )
      storeEntries.sort( ( [aKey, aValue], [bKey, bValue] ) => aValue - bValue )
      delete this._store[storeEntries[0][0]]
    }
...
function solution( cacheSize: number, cities: string[] ) {
  ...
  cities.forEach( ( cityName ) => {
    answer += cache.dbGet( cityName.toLowerCase() )
  } )
